...
const PORTAL_BUYOUT_DISCOUNT = 0.10; // быстрый выкуп: рынок минус 10%
...
function runPortalBuyoutCalc() {
  ...
  const discountPct = Math.round(PORTAL_BUYOUT_DISCOUNT * 100);

  if (!car.brand || !car.model || !year) {
    el.innerHTML = '<p class="calc-note">Выберите марку, модель и год — расчёт рыночной цены и выкупа появится сразу.</p>';
    return;
  }

  const { market } = calcMarketPrice(car.brand, car.model, year, mileage, condition);
  const cut = Math.round(market * PORTAL_BUYOUT_DISCOUNT / 5000) * 5000;
  const buyout = Math.round(market * (1 - PORTAL_BUYOUT_DISCOUNT) / 5000) * 5000;
  const kmLabel = mileage ? Number(String(mileage).replace(/\s/g, '')).toLocaleString('ru-RU') + ' км' : 'пробег не указан';

  el.innerHTML =
    '<div class="buyout-box">' +
    '<div class="result-row"><span>Автомобиль</span><strong>' + car.brand + ' ' + car.model + ', ' + year + '</strong></div>' +
    '<div class="result-row"><span>Пробег / состояние</span><strong>' + kmLabel + ' · ' + condition + '</strong></div>' +
    '<div class="result-row"><span>Средняя цена Дром / Авто.ру</span><strong>' + fmt(market) + '</strong></div>' +
    '<div class="result-row buyout-cut"><span>Минус ' + discountPct + '% за быстрый выкуп</span><strong>− ' + fmt(cut) + '</strong></div>' +
    '<div class="result-total"><span>Цена быстрого выкупа</span><strong>' + fmt(buyout) + '</strong></div>' +
    '<p class="calc-note">Ориентир по средним ценам площадок. Точная сумма — после осмотра в Иркутске.</p>' +
    '</div>';
}
